...
function parseLocked(input){
  if (!input) return [];
  return input.split(/[\s,]+/).map(s=>Number(s)).filter(n=>Number.isInteger(n) && n>=1 && n<=45);
}

function generateWithLocks(locks, mode, freq){
  const uniq = Array.from(new Set(locks)).slice(0,6);
  let remaining = 6 - uniq.length;
  if (remaining <= 0) return uniq.sort((a,b)=>a-b);
  const poolSet = new Set(uniq);
  const add = (n)=>{ if(!poolSet.has(n)){ poolSet.add(n); return true;} return false; };
  while (remaining>0){
    const nums = (mode==='pure') ? pickPure(remaining) : weightedPick(freq, mode, remaining);
    for (const n of nums) { if(add(n)) remaining--; if (remaining===0) break; }
  }
...
(async function main(){
  ...
  const gen = () => {
    ...
    for (let i=0;i<cnt;i++) {
      const locks = parseLocked(locked && locked.value);
      const nums = generateWithLocks(locks, mode, freq);
